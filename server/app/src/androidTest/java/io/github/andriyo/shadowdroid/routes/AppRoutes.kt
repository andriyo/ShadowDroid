    ...
    fun register(route: Route, uiDevice: UiDevice, instr: Instrumentation) {
        ...
        route.post("/app/wait") {
            ...
            while (System.currentTimeMillis() < deadline) {
                ...
                lastPkg = cur
                val match = if (r.front) cur == r.`package`
                            else (cur == r.`package` || pidForPackage(uiDevice, r.`package`) != null)
                if (match) {
        ...
        route.get("/app/info") {
            ...
            val versionName = Regex("""versionName=(.+)""").find(out)?.groupValues?.get(1)?.trim()
            val versionCode = Regex("""versionCode=(\d+)""").find(out)?.groupValues?.get(1)?.toIntOrNull()
...
@Serializable
private data class PkgReq(val `package`: String)
...
@Serializable
private data class AppWaitResp(val matched: Boolean, val current: String? = null)
