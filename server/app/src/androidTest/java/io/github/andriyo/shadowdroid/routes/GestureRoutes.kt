...
object GestureRoutes {
    ...
    fun register(route: Route, uiDevice: UiDevice) {
        ...
        route.post("/swipe_ext") {
            ...
            val (x1, y1, x2, y2) = swipeExtCoords(
                uiDevice.displayWidth, uiDevice.displayHeight, r.direction, r.scale
            )
            val steps = (r.duration_ms / 5).coerceAtLeast(1)
 ...
 *  Returns (x1, y1, x2, y2). */
private fun swipeExtCoords(w: Int, h: Int, dir: String, scale: Float): Quadruple<Int, Int, Int, Int> {
    val cx = w / 2
    val cy = h / 2
    ...
    return when (dir.lowercase()) {
        ...
        else -> throw BadRequest("bad_direction",
...
@Serializable
private data class XyReq(val x: Int, val y: Int)

@Serializable
private data class LongTapReq(val x: Int, val y: Int, val duration_ms: Int = 600)

@Serializable
private data class SwipeReq(val from: List<Int>, val to: List<Int>, val duration_ms: Int = 200)
