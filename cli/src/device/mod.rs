...
//! - `adb` — talk to the host `adbd` over the ADB protocol (port 5037), not by
//!   shelling out. Used for forward/install/push/pull/logcat.
//! - `client` — HTTP client to the on-device ShadowDroid server (port 7912 after
//!   `adb forward`), used for every UI operation.
//!
