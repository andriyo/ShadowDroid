...
// Inlined into every agent session's system prompt (skill name + description),
// whether or not the session touches Android — keep it short, but keep the
// routing trigger nouns: recall beats brevity here. Behavior-steering prose
// (e.g. "Gradle remains the build engine") belongs in the body instead.
const DESCRIPTION: &str = "Drive Android devices and apps via the `shadowdroid` CLI with \
structured JSON: deploy, UI automation, logs/crashes, debugger and layout inspection, \
network/OkHttp capture and intercept, permissions, files. Use for Android development, \
...
fn append_marker(core: &str) -> String {
    format!(
        "{core}{MARKER_PREFIX}v={v} h={h} · managed by `shadowdroid skill --sync`; hand-edits are preserved. -->\n",
...
///
/// Core groups get a one-liner each; members are deliberately not expanded —
/// the machine catalog is the source of truth for member-level discovery, and
/// an inline copy would both cost ~6 KB of context per invocation and drift
/// stale between syncs. Still generated from the live catalog, so group names
/// and summaries never drift from the actual CLI.
fn command_reference() -> String {
    // The core driving surface, one line per group. Anything else is listed by
    // name only. Matched against the live catalog, so a renamed verb falls
    // through to the pointer line rather than silently vanishing.
    const CORE: &[&str] = &[
        "devices",
        "connect",
        "disconnect",
    ...
    let mut out = String::from(
        "## Command reference\n\nCore groups below. Run \
         `shadowdroid commands --json --describe '<group> <sub>'` before \
         constructing any command; `commands --json --depth 1` lists every \
         member.\n\n",
    );
    out.push_str(&core);
    if !tail.is_empty() {
...
fn first_sentence(about: &str) -> &str {
    ...
    for terminator in [". ", "? ", "! "] {
        if let Some(i) = about.find(terminator) {
            end = end.min(i + 1);
        }
    }
    &about[..end]
...
mod tests {
    ...
    #[test]
    fn command_reference_lists_core_groups_and_points_to_catalog_for_members() {
        let r = command_reference();
        // Core driving groups appear as one-liners…
        assert!(r.contains("- **`ui`**"), "{r}");
        assert!(r.contains("- **`watch`**"), "{r}");
        assert!(r.contains("- **`net`**"), "{r}");
        // …with no member expansion — member discovery belongs to the catalog.
        assert!(!r.contains("  - `"), "{r}");
        assert!(!r.contains("`ui tap`"), "{r}");
        // The advanced long tail is named but not expanded…
        assert!(!r.contains("debug variables"), "{r}");
        assert!(r.contains("debug"));
        // …and both tiers point at the machine catalog.
        assert!(r.contains("commands --json --describe"), "{r}");
        assert!(r.contains("commands --json --depth 1"), "{r}");
    }

    /// Ratchet guard for the skill's context cost (issues #26–#31): the
    ...
    fn generated_skill_stays_within_size_budget() {
        ...
        assert!(
            ...
            "SKILL.md grew to {} bytes (~{} tokens); trim skill_body.md or move \
             domain depth behind `commands --guide` (cli/src/cmd/introspect/guides/) \
             instead of adding always-loaded prose",
            content.len(),
            content.len() / 4
        );
    }

    /// The always-loaded body must keep routing to every on-demand guide —
    /// depth was moved out of the skill, not deleted.
    #[test]
    fn skill_body_points_at_every_domain_guide() {
        let topics = crate::cmd::introspect::guide_topics();
        assert!(!topics.is_empty());
        for topic in topics {
            assert!(
                SKILL_BODY.contains(&format!("--guide {topic}")),
                "skill_body.md lost its pointer to `commands --guide {topic}`"
            );
        }
    }
